...
def _dir(obj: object, modules: Set[object], classes: Set[object], functions: Set[object]):
    if inspect.ismodule(obj):
        if obj.__name__.startswith(PREFIX) and obj not in modules:
            modules.add(obj)
            for child in dir(obj):
                _dir(getattr(obj, child), modules, classes, functions)
    elif inspect.isclass(obj):
        if obj not in classes:
            if not obj.__name__.startswith('_') and obj.__module__.startswith(f'{PREFIX}.'):
                classes.add(obj)
...
def generate_index() -> None:
    ...
    _dir(stpr, modules, classes, functions)
